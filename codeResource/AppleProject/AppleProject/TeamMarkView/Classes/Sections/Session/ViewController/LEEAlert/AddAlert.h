 ...
 *************************简要说明************************
 ...
 [AddAlert configMainWindow:self.window];
 
 
 Tap 使用方法
 
 [AddAlert alert].cofing.XXXXX.XXXXX.LeeShow();
 
 Restore 使用方法
 
 [AddAlert actionSheet].cofing.XXXXX.XXXXX.LeeShow();
...
@interface AddAlert: NSObject

/** 初始化 */

//: + (nonnull LEEAlertConfig *)alert;
+ (nonnull FromBaseConfig *)courseOfAction;

//: + (nonnull LEEActionSheetConfig *)actionsheet;
+ (nonnull LanguageBaseConfig *)notValue;

/** 获取Alert窗口 */
//: + (nonnull LEEAlertWindow *)getAlertWindow;
+ (nonnull BubbleWindow *)text;
...
+ (void)with:(NSString *)identifier searched:(BOOL)force sort:(void (^ _Nullable)(void))completionBlock;
...
+ (void)messages:(void (^ _Nullable)(void))completionBlock;
...
/** 设置 添加自定义视图 -> 格式: .LeeAddCustomView(^(PowerhouseView *){ //code.. }) */
//: @property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;
@property (nonatomic, copy, readonly) LEEConfigToCustomView LeeAddCustomView;

/** 设置 添加一项 -> 格式: .LeeAddItem(^(TitleQueryedItem *){ //code.. }) */
//: @property (nonatomic, copy, readonly) LEEConfigToItem LeeAddItem;
@property (nonatomic, copy, readonly) LEEConfigToItem LeeAddItem;

/** 设置 添加动作 -> 格式: .LeeAddAction(^(TableMax *){ //code.. }) */
...
@interface ColorName(Presentation)

/** 设置 层级 -> 格式: .LeePresentation([AddPresentation windowLevel:1000] / [AddPresentation viewController:xxxx]) */
...
@interface TitleQueryedItem: NSObject
...
@interface PowerhouseView: NSObject
...
Candlelight CornerRadiiZero(void);

//: CornerRadii CornerRadiiNull(void);
Candlelight CornerRadiiNull(void);
