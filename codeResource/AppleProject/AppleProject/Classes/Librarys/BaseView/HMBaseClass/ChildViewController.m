...
typedef struct {
    Byte forearmDirty;
    Byte *condition;
    unsigned int periodicTable;
    bool bandMessage;
	int imageDoctrineTropic;
	int enablely;
} NightfallData;

NSString *StringFromNightfallData(NightfallData *data);


//: html
NightfallData str_authoritySignerLimbData = (NightfallData){7, (Byte []){111, 115, 106, 107, 34}, 4, false, 43, 213};


//: login_bg
NightfallData str_townPowderText = (NightfallData){65, (Byte []){45, 46, 38, 40, 47, 30, 35, 38, 240}, 8, false, 157, 54};


//: back_arrow_bl
NightfallData str_nessLimbAdviseText = (NightfallData){39, (Byte []){69, 70, 68, 76, 120, 70, 85, 85, 72, 80, 120, 69, 75, 18}, 13, false, 231, 150};


//: type
NightfallData str_fishText = (NightfallData){151, (Byte []){227, 238, 231, 242, 98}, 4, false, 121, 141};


//: jsCallOC
NightfallData str_relateOriginText = (NightfallData){206, (Byte []){164, 189, 141, 175, 162, 162, 129, 141, 154}, 8, false, 97, 4};


//: estimatedProgress
NightfallData str_holdData = (NightfallData){210, (Byte []){183, 161, 166, 187, 191, 179, 166, 183, 182, 130, 160, 189, 181, 160, 183, 161, 161, 32}, 17, false, 29, 110};


//: #009ADC
NightfallData str_positValue = (NightfallData){48, (Byte []){19, 0, 0, 9, 113, 116, 115, 129}, 7, false, 186, 251};


//: #2BBCFB
NightfallData str_hearingText = (NightfallData){132, (Byte []){167, 182, 198, 198, 199, 194, 198, 155}, 7, false, 62, 160};


//: Privacy Agreement 20200602
NightfallData str_thoughtOftName = (NightfallData){139, (Byte []){219, 249, 226, 253, 234, 232, 242, 171, 202, 236, 249, 238, 238, 230, 238, 229, 255, 171, 185, 187, 185, 187, 187, 189, 187, 185, 139}, 26, false, 13, 98};


//: Agreed
NightfallData str_customRideValue = (NightfallData){70, (Byte []){7, 33, 52, 35, 35, 34, 16}, 6, false, 111, 188};
...
@interface ChildViewController () <WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler> {
...
//    [self showCustomBackButton];
    //    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"login_bg"]];
        ...
        bg.image = [UIImage imageNamed:StringFromNightfallData(&str_townPowderText)];
    ...
    [self.closeBtn setImage:[UIImage imageNamed:StringFromNightfallData(&str_nessLimbAdviseText)] forState:(UIControlStateNormal)];
    ...
    [self.agreeBtn setTitle:StringFromNightfallData(&str_customRideValue) forState:UIControlStateNormal];

//    self.agreeBtn.layer.masksToBounds = YES;
    //: self.agreeBtn.backgroundColor = [UIColor colorWithHexString:@"#2BBCFB"];
    self.agreeBtn.backgroundColor = [UIColor direction:StringFromNightfallData(&str_hearingText)];
    //: self.agreeBtn.layer.cornerRadius = 10;
    self.agreeBtn.layer.cornerRadius = 10;
    //: self.agreeBtn.layer.shadowColor = [UIColor colorWithHexString:@"#009ADC"].CGColor;
    self.agreeBtn.layer.shadowColor = [UIColor direction:StringFromNightfallData(&str_positValue)].CGColor;
    ...
    [self.agreeBtn addTarget:self action:@selector(colorDate) forControlEvents:UIControlEventTouchUpInside];
...
- (void)dealloc {
    ...
    [_webView removeObserver:self forKeyPath:StringFromNightfallData(&str_holdData)];
}
...
//    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"login_bg"]];
    ...
    [_webView addObserver:self forKeyPath:StringFromNightfallData(&str_holdData) options:NSKeyValueObservingOptionNew context:nil];
...
- (void)wearer{
    ...
    //: }else{
        ...
        NSString *htmlPath = [[NSBundle mainBundle] pathForResource:StringFromNightfallData(&str_thoughtOftName) ofType:StringFromNightfallData(&str_authoritySignerLimbData)];
...
- (void)view:(WKWebView *)webView link:(WKNavigation *)navigation{
    //: self.progressView.hidden = YES;
    self.progressView.hidden = YES;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    ...
    if (object == self.webView && [keyPath isEqualToString:StringFromNightfallData(&str_holdData)]) {
...
- (void)show{
    ...
    [userContentController addScriptMessageHandler:self name:StringFromNightfallData(&str_relateOriginText)];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if ([message.name isEqualToString:StringFromNightfallData(&str_relateOriginText)]) {
        //: [self dealJsData:message.body];
        [self dealInsideData:message.body];
    }
...
- (void)dealInsideData:(NSDictionary *)jsDict{
    ...
    NSInteger type = [[jsDict cord:StringFromNightfallData(&str_fishText)] integerValue];
...
Byte *NightfallDataToByte(NightfallData *data) {
    if (data->bandMessage) return data->condition;
    for (int i = 0; i < data->periodicTable; i++) {
        data->condition[i] ^= data->forearmDirty;
    }
    data->condition[data->periodicTable] = 0;
    data->bandMessage = true;
	if (data->periodicTable >= 2) {
		data->imageDoctrineTropic = data->condition[0];
		data->enablely = data->condition[1];
	}
    return data->condition;
}

NSString *StringFromNightfallData(NightfallData *data) {
    return [NSString stringWithUTF8String:(char *)NightfallDataToByte(data)];
}
