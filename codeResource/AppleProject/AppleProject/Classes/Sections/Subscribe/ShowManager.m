...
NSString *const k_formatValue = @"online_state";
...
@interface ShowManager()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
+ (instancetype)error
{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        instance = [[ShowManager alloc] init];
    //: });
    });
    //: return instance;
    return instance;
...
- (void)getToGrips:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self title];
...
- (void)message
{
    ...
    NSDictionary *ext = @{
                            ...
                            k_styleValue : @([StateDevice straddle].kit),
                            //: NTESSubscribeOnlineState : @(NTESOnlineStateNormal), 
                            k_formatValue : @(NTESOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext tingName]];
...
- (void)searched
{
    ...
    [_subscribeIds addObjectsFromArray:self.messageAcrossIds.allObjects];
    ...
    [self view:_subscribeIds.allObjects];
}

//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)view:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (!isMyFriend && ![self.messageAcrossIds containsObject:user.userId]) {
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.at containsObject:recentSession.session.sessionId]) {
