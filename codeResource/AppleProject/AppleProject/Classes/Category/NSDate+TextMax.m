...
NSString *StringFromBreakfastData(Byte *data);


//: %d分钟前
Byte str_analNaturalProgressText[] = {33, 11, 56, 8, 24, 207, 118, 236, 93, 156, 29, 192, 190, 33, 202, 215, 29, 193, 197, 151};


//: %d小时前
Byte str_combineDeveloperName[] = {52, 11, 99, 9, 119, 78, 5, 104, 150, 136, 199, 72, 19, 242, 73, 250, 25, 72, 236, 240, 153};


//: yyyy-MM-dd
Byte str_holdTitle[] = {42, 10, 94, 5, 117, 215, 215, 215, 215, 139, 171, 171, 139, 194, 194, 131};


//: yyyy.MM.dd更新
Byte str_typicallyNearName[] = {99, 16, 29, 7, 192, 244, 189, 150, 150, 150, 150, 75, 106, 106, 75, 129, 129, 3, 184, 209, 3, 179, 205, 33};


//: before_yesterday
Byte str_vendJoinText[] = {49, 16, 6, 5, 148, 104, 107, 108, 117, 120, 107, 101, 127, 107, 121, 122, 107, 120, 106, 103, 127, 121};


//: %d小时前更新
Byte str_ratedChemicalTitle[] = {23, 17, 95, 5, 169, 132, 195, 68, 15, 238, 69, 246, 21, 68, 232, 236, 69, 250, 19, 69, 245, 15, 112};


//: %d分钟前更新
Byte str_screwData[] = {29, 17, 34, 13, 42, 72, 187, 116, 135, 144, 22, 149, 101, 71, 134, 7, 170, 168, 11, 180, 193, 7, 171, 175, 8, 189, 214, 8, 184, 210, 96};


//: yesterday
Byte str_thoughtData[] = {15, 9, 45, 13, 130, 50, 165, 128, 227, 231, 45, 50, 163, 166, 146, 160, 161, 146, 159, 145, 142, 166, 217};
...
+ (NSString *)roundInfo:(NSDate*)date {
    ...
    return [dateFormatter model:date];
}
...
+ (NSString *)ting:(NSDate*)date {
    ...
    return [dateFormatter imageOrInput:date];
}
...
+ (NSString *)duringTo:(NSDate*)date {
    ...
    return [dateFormatter solarDay:date];
}

//: + (NSCalendar *)calendar:(NSDate*)date {
+ (NSCalendar *)add:(NSDate*)date {
...
+ (int)argument:(NSDate*)date {
    ...
    return (int)[[NSDate add:date] rangeOfUnit:NSCalendarUnitDay
...
+ (int)view:(NSDate*)date {
    ...
    return (int)[[NSDate add:date] rangeOfUnit:NSCalendarUnitWeekOfMonth
                                             //: inUnit:NSCalendarUnitMonth
                                             inUnit:NSCalendarUnitMonth
                                            //: forDate:date].length;
                                            forDate:date].length;
...
+ (NSString*)pollToFormatter:(NSString*)oldDateString
    ...
    NSDate *oldDate = [NSDate ratioFormatter:oldDateString info:oldFormatterString];

    //: NSString *returnString = [NSDate getStringFromDate:oldDate dateFormatter:newFormatterString];
    NSString *returnString = [NSDate isText:oldDate omitFor:newFormatterString];
...
- (NSString *)scaleOfMeasurement
{
    ...
    if ([components day] >= 3) {
        ...
        return [NSDate isText:self omitFor:StringFromBreakfastData(str_holdTitle)];
    //: } else if ([components day] >= 2) {
    } else if ([components day] >= 2) {
        //: return [NTESLanguageManager getTextWithKey:@"before_yesterday"];
        return [MaxInformation off:StringFromBreakfastData(str_vendJoinText)];
    //: } else if ([components day] >= 1) {
    } else if ([components day] >= 1) {
        //: return [NTESLanguageManager getTextWithKey:@"yesterday"];
        return [MaxInformation off:StringFromBreakfastData(str_thoughtData)];
    //: } else if ([components hour] > 0) {
    } else if ([components hour] > 0) {
        //: return [NSString stringWithFormat:@"%d小时前", (int)[components hour]];
        return [NSString stringWithFormat:StringFromBreakfastData(str_combineDeveloperName), (int)[components hour]];
    //: } else if ([components minute] > 0) {
    } else if ([components minute] > 0) {
        //: return [NSString stringWithFormat:@"%d分钟前", (int)[components minute]];
        return [NSString stringWithFormat:StringFromBreakfastData(str_analNaturalProgressText), (int)[components minute]];
...
- (NSString *)tableDHote
{
    ...
    if ([components day] >= 1) {
        ...
        return [NSDate isText:self omitFor:StringFromBreakfastData(str_typicallyNearName)];
    //: } else if ([components hour] > 0) {
    } else if ([components hour] > 0) {
        //: return [NSString stringWithFormat:@"%d小时前更新", (int)[components hour]];
        return [NSString stringWithFormat:StringFromBreakfastData(str_ratedChemicalTitle), (int)[components hour]];
    //: } else if ([components minute] > 0) {
    } else if ([components minute] > 0) {
        //: return [NSString stringWithFormat:@"%d分钟前更新", (int)[components minute]];
        return [NSString stringWithFormat:StringFromBreakfastData(str_screwData), (int)[components minute]];
    //: }else {
    }else {
        //: return @"刚刚";
        return @"刚刚";
...
Byte * BreakfastDataToCache(Byte *data) {
    int hold = data[0];
    int thigh = data[1];
    Byte countroduceRitual = data[2];
    int easyMax = data[3];
    if (!hold) return data + easyMax;
    for (int i = easyMax; i < easyMax + thigh; i++) {
        int value = data[i] - countroduceRitual;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[easyMax + thigh] = 0;
    return data + easyMax;
}

NSString *StringFromBreakfastData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)BreakfastDataToCache(data)];
}
