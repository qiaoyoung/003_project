...
//    [self performSelector:@selector(selectAll:) withObject:nil afterDelay:0.0];
    ...
    CGRect tempRect = [self convertRect:resultRect toView:((FromDelegate*)([UIApplication sharedApplication].delegate)).window];
    ...
    CGRect cursorStartRectToWindow = [self convertRect:startRect toView:((FromDelegate*)([UIApplication sharedApplication].delegate)).window];
...
- (void)infoMonth:(NIMMessage *)message {
    ...
    [items enumerateObjectsUsingBlock:^(WithMin *item, NSUInteger idx, BOOL *stop) {
        ...
        if (item.selctor == @selector(replied:)) {
