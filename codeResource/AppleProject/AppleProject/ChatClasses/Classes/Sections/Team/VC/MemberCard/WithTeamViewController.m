...
- (NSArray *)chromatic{
    ...
    NSDictionary *headerItem = @{
                                 ...
                                 StringFromEstheticData(str_anotherLeaderValue) : @{StringFromEstheticData(str_visionName):usrInfo, StringFromEstheticData(str_showLeaderRideData):@(_member.at)},
                                 //: @"leftEdge" : @(15)
    ...
    NSDictionary *userTypeItem = @{
                                   ...
                                   StringFromEstheticData(str_twoSessionValue) : [LanguageChild futurism:self.member.at],
    ...
    NSDictionary *inviterAccidItem = @{
                                       ...
                                       StringFromEstheticData(str_twoSessionValue) : _member.arrayAccid ? (_member.arrayAccid.length ? _member.arrayAccid : _member.userId) : StringFromEstheticData(str_jurorChickenValue).trapLocalized,
                                       //: @"action" : @"",
    ...
    NSDictionary *isMuteItem = @{
                                  ...
                                  StringFromEstheticData(str_anotherLeaderValue) : @(_member.mediaSize),
                                  //: @"leftEdge" : @(15),
                                  StringFromEstheticData(str_arcData) : @(15),
                                  //: };
                                  };
    ...
    if (_member.greenOf == NIMTeamTypeSuper) {
...
- (void)link
{
    ...
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:[MaxInformation off:StringFromEstheticData(str_imageIncreasingName)]
                                         ...
                                         destructiveButtonTitle:nil
                                              ...
                                              otherButtonTitles: self.member.at == NIMTeamMemberTypeManager ? [MaxInformation off:StringFromEstheticData(str_cartScanText)] : [MaxInformation off:StringFromEstheticData(str_recordValueText)], nil];
...
- (void)exceptText:(UIAlertView *)alertView success:(NSInteger)buttonIndex{
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex{
    ...
    if(buttonIndex == 0) {
        ...
        NIMTeamMemberType userType = self.member.at;
...
- (BOOL)path {
    ...
    return self.viewer.at == NIMTeamMemberTypeOwner;
}
...
- (BOOL)color {
    ...
    BOOL viewerIsManager = self.viewer.at == NIMTeamMemberTypeManager;
    //: BOOL memberIsNormal = self.member.userType == NIMTeamMemberTypeNormal;
    BOOL memberIsNormal = self.member.at == NIMTeamMemberTypeNormal;
    //: if (viewerIsOwner) {
    if (viewerIsOwner) {
        //: ret = ![self isSelf];
