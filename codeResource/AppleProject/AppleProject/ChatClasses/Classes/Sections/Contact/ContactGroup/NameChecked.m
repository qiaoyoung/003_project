...
- (instancetype)initWithTransactionPlace:(id)first itemId:(id)second {
    //: self = [super init];
    self = [super init];
    //: if(self) {
    if(self) {
...
//: @interface AAAAppleProjectGroupedData ()
@interface NameChecked ()
...
//: @implementation AAAAppleProjectGroupedData
@implementation NameChecked
...
- (void)setMembers:(NSArray *)members {
    ...
    for (id<UpProtocol>member in members) {
        //: if ([[member memberId] isEqualToString:me]) {
        if ([[member video] isEqualToString:me]) {
            //: continue;
            continue;
        }
        //: NSString *groupTitle = [member groupTitle];
        NSString *groupTitle = [member team];
    ...
    [tmp enumerateKeysAndObjectsUsingBlock:^(NSString *groupTitle, NSMutableArray *groupedMembers, BOOL *stop) {
        ...
        if (groupTitle.length) {
            ...
            [_groups addObject:[[RangePair alloc] initWithTransactionPlace:groupTitle itemId:groupedMembers]];
        }
    //: }];
    }];
    //: [self sortGroup];
...
- (void)setSpecialMembers:(NSArray *)specialMembers {
    ...
    if (specialMembers.count > 0) {
        ...
        [_groups insertObject:[[RangePair alloc] initWithTransactionPlace:@"$" itemId:specialMembers] atIndex:0];
...
- (NSDictionary *)contentDic {
    ...
    for (int i = 0; i < _groups.count; ++i) {
        ...
        NSArray *tempArr = [self past:i];
        //: NSString *title = [self titleofGroup:i];
        NSString *title = [self session:i];
        //: [tempArr enumerateObjectsUsingBlock:^(id<NIMGroupMemberProtocol>member, NSUInteger idx, BOOL * _Nonnull stop) {
        [tempArr enumerateObjectsUsingBlock:^(id<UpProtocol>member, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSString *)session:(NSInteger)groupIndex {
...
- (NSArray *)past:(NSInteger)groupIndex
{
    //: if(groupIndex >= 0 && groupIndex < _groups.count) {
    if(groupIndex >= 0 && groupIndex < _groups.count) {
        //: NIMKitPair *pair = [_groups objectAtIndex:groupIndex];
        RangePair *pair = [_groups objectAtIndex:groupIndex];
        //: return pair.second;
        return pair.second;
    }
    //: return nil;
...
- (void)disable
{
    ...
    [_groups sortUsingComparator:^NSComparisonResult(RangePair *pair1, RangePair *pair2) {
...
- (void)infoMessage
{
    ...
    [_groups enumerateObjectsUsingBlock:^(RangePair *obj, NSUInteger idx, BOOL *stop) {
        ...
        [groupedMembers sortUsingComparator:^NSComparisonResult(id<UpProtocol> member1, id<UpProtocol> member2) {
            //: return _groupMemberComparator([member1 sortKey], [member2 sortKey]);
            return _groupMemberComparator([member1 artificialFlowerStemWithKey], [member2 artificialFlowerStemWithKey]);
        //: }];
        }];
    //: }];
    }];
