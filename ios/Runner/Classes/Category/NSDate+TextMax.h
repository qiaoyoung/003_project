...
@interface NSDate (TextMax)

/*日期*/
//: + (NSString *)day:(NSDate*)date;
+ (NSString *)roundInfo:(NSDate*)date;

/*星期几*/
//: + (NSString *)weekday:(NSDate*)date;
+ (NSString *)perimeter:(NSDate*)date;
...
+ (NSString *)duringTo:(NSDate*)date;

/*获取当前月有多少天*/
//: + (int)numberOfDaysInMonth:(NSDate*)date;
+ (int)argument:(NSDate*)date;

/*获取当前月有多少周*/
//: + (int)weeksOfMonth:(NSDate*)date;
+ (int)view:(NSDate*)date;

/*是否在某个日期之前*/
//: - (BOOL)isBefore:(NSDate *)date;
- (BOOL)flush:(NSDate *)date;

/*是否在某个日期之后*/
//: - (BOOL)isAfter:(NSDate *)date;
- (BOOL)message:(NSDate *)date;

/*是否在某个时间闭区间之间*/
//: - (BOOL)isBetween:(NSDate *)startDate and:(NSDate *)endDate;
- (BOOL)shouldAnd:(NSDate *)startDate date:(NSDate *)endDate;
...
+ (BOOL)button:(NSDate*)date01 barScript:(NSUInteger)days;
...
+ (NSDate*)image:(NSString*)dateString selected:(NSString*)formatterString;

//: + (NSString*)getStringFromOldDateString:(NSString*)oldDateString
+ (NSString*)pollToFormatter:(NSString*)oldDateString
                       //: withOldFormatter:(NSString*)oldFormatterString
                       replacement:(NSString*)oldFormatterString
                           //: newFormatter:(NSString*)newFormatterString;
                           imageBubble:(NSString*)newFormatterString;

//: + (NSString*)getStringFromDate:(NSDate*)date dateFormatter:(NSString*)formatterString;
+ (NSString*)isText:(NSDate*)date omitFor:(NSString*)formatterString;

//: + (NSDate*)getDateFromString:(NSString*)string dateFormatter:(NSString*)formatterString;
+ (NSDate*)ratioFormatter:(NSString*)string info:(NSString*)formatterString;


//: + (BOOL)isDate:(NSDate*)date01 earlierThanDate:(NSDate*)date02;
+ (BOOL)successDate:(NSDate*)date01 marginImage:(NSDate*)date02;
...
+ (BOOL)color:(NSString*)date1String01 key02:(NSString*)date1String02 show:(NSString*)formatter01 cell:(NSString*)formatter02;

//: + (BOOL)isString:(NSString*)date1String01 earlierThanDate:(NSDate*)date02 formatter01:(NSString*)formatter01;
+ (BOOL)twentyFourHourPeriod:(NSString*)date1String01 by:(NSDate*)date02 can:(NSString*)formatter01;

//: + (NSDate *)dateForTodayInClock:(NSInteger)clock;
+ (NSDate *)info:(NSInteger)clock;

//: - (NSString *)normalizeDateString;
- (NSString *)scaleOfMeasurement;
