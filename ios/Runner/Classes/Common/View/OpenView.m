...
@interface PanoramaScrollView : UIScrollView
...
- (void)ting
{
    ...
    if (_scrollView == nil)
    {
        ...
        _scrollView = [[PanoramaScrollView alloc]initWithFrame:self.bounds];
...
- (void)teamSession: (NSInteger)page
{
...
- (NSInteger)view:(NSInteger)value usufructuary:(NSInteger)min minPost:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)forPage:(NSInteger)currentPage
{
    ...
    NSInteger first = [self view:currentPage - 1 usufructuary:0 minPost:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self view:currentPage + 1 usufructuary:0 minPost:count - 1];
...
@implementation PanoramaScrollView
//fix: swipe to delete in a UITableView which is embeded in a UIScrollView
//: - (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRequireFailureOfGestureRecognizer:(nonnull UIGestureRecognizer *)otherGestureRecognizer
- (BOOL)empty:(UIGestureRecognizer *)gestureRecognizer disableName:(nonnull UIGestureRecognizer *)otherGestureRecognizer
{
